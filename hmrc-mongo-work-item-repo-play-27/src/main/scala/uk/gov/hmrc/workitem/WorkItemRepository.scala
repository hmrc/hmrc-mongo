...
* Why does pushNew take a `receivedAt` field, when implementers have already had to define `def now: DateTime` ? In fact, receivedAt is never used...
README says: "Enables a microservice to consume work generated by another service."
  Is this true!? Would require more than one service to share same repository - a pattern not recommended... Is it more probable to enable a microservice to share a workload amongst it's own instances?
...
abstract class WorkItemRepository[T, ID](
  ...
  /** Returns the property key which defines the millis in Long format, for the [[inProgressRetryAfter]]
    ...
    */
  // TODO replace with Duration, and update name of config key (add `work-item-repo` prefix)
