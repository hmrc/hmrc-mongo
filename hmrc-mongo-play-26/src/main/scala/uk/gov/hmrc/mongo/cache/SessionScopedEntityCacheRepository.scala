...
/** A variation of [[EntityCacheRepository]] where the key is the sessionId.
*/
class SessionScopedEntityCacheRepository[A: ClassTag] @Inject()(
  ...
  private val cacheRepo = new EntityCacheRepository(
      mongoComponent   = mongoComponent,
  ...
  private def cacheId(implicit request: Request[Any]): CacheId =
    request.session.get("sessionId").map(CacheId.apply).getOrElse(throw NoSessionException)

  def put(body: A)(implicit request: Request[Any]): Future[Unit] =
    cacheRepo.put(cacheId, body)

  def get()(implicit request: Request[Any]): Future[Option[A]] =
    cacheRepo.get(cacheId)

  def delete()(implicit request: Request[Any]): Future[Unit] =
    cacheRepo.delete(cacheId)
}
