...
@Singleton
class MongoComponentImpl @Inject()(
  ...
  val connection: ConnectionString = new ConnectionString(mongoConfig.uri)

  val client: MongoClient = MongoClient(uri = mongoConfig.uri)

  val database: MongoDatabase = client.getDatabase(connection.getDatabase)
